...
#define NUMBER_OF_RAYS 3000000  // approximate count
...
hit(struct application *ap, struct partition *PartHeadp, struct seg *UNUSED(segs))
{
    ...
    if (pp != PartHeadp) {  /* Ensure we have a partition */
        ...
        const char* filename = "ray_info_ex.txt";
...
main(int argc, char **argv)
{
    ...
    vect_t bbox_dimensions;
    VSUB2(bbox_dimensions, rtip->mdl_max, rtip->mdl_min);

    // Compute half of the diagonal length of the bounding box
    double half_diagonal = 0.5 * MAGNITUDE(bbox_dimensions);

    // Optionally add some margin. This can be a small value to ensure rays are just outside the box.
    double margin = 5;  // adjust as needed

    // Set d
    double d = half_diagonal + margin;

    for (int i = 0; i < NUMBER_OF_RAYS; i++) {
        double theta = 2 * M_PI * i / GOLDEN_RATIO;
        double phi = acos(1 - 2 * (i + 0.5) / NUMBER_OF_RAYS);

        // Convert spherical coordinates to Cartesian coordinates
        point_t ray_origin;
        ray_origin[0] = center[0] + d * sin(phi) * cos(theta);
        ray_origin[1] = center[1] + d * sin(phi) * sin(theta);
        ray_origin[2] = center[2] + d * cos(phi);

        // Direction towards center
        VSUB2(ray_dir, center, ray_origin);
        VUNITIZE(ray_dir);

        // Set ray application specifics
        VMOVE(ap.a_ray.r_pt, ray_origin);
