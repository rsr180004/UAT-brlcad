...
class HitClassifier(nn.Module):
    ...
    def load_model1_weights(self, path):
        ...
        # print("printing expected keys")
        # for key, val in self.state_dict.items():
        #     print(key)

        # print("LOADING weights at path: ", path)
        ...
        for key, val in loaded_state_dict.items():
            if "layer1.weight" in key:
                new_state_dict["model.0.weight"] = val
            elif "layer1.bias" in key:
                new_state_dict["model.0.bias"] = val
            elif "layer2.weight" in key:
                new_state_dict["model.2.weight"] = val
            elif "layer2.bias" in key: 
                new_state_dict["model.2.bias"] = val
            elif "layer3.weight" in key:
                new_state_dict["model.4.weight"] = val
            elif "layer3.bias" in key:
                new_state_dict["model.4.bias"] = val
            elif "layer4.weight" in key:
                new_state_dict["model.6.weight"] = val
            elif "layer4.bias" in key:
                new_state_dict["model.6.bias"] = val
            elif "output.weight" in key:
                new_state_dict["model.8.weight"] = val
            elif "output.bias" in key:
                new_state_dict["model.8.bias"] = val
            elif "0.weight" in key:
                new_state_dict["model.0.weight"] = val
            elif "0.bias" in key:
                new_state_dict["model.0.bias"] = val
                ...
                new_state_dict["model.4.bias"] = val
            elif "6.weight" in key:
                new_state_dict["model.6.weight"] = val
            elif "6.bias" in key:
                new_state_dict["model.6.bias"] = val
            elif "8.weight" in key:
                new_state_dict["model.8.weight"] = val
            elif "8.bias" in key:
                new_state_dict["model.8.bias"] = val
            else:
        ...
        try:
            ...
            self.eval()
            # print("WEIGHTS ARE LOADED LOADED!")
        except Exception as e:
...
def train_model_1(model_path):
    ...
    # print("in train neural net function!")
    ...
    model1 = nn.Sequential(
            nn.Linear(5, 120),
            #nn.BatchNorm1d(480),  # Batch normalization after linear layer
            nn.ReLU(),
            #nn.Dropout(0.2),
            nn.Linear(120, 80),
            nn.ReLU(),
...
def train_model_2(model_path):
    ...
    for n in range(num_epochs):
        ...
        #print(f"Training time for Epoch {n+1}/{num_epochs}: {end_time_train - start_time_train:.2f} seconds")
