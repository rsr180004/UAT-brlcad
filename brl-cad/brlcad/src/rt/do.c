...
do_frame(int framenumber, int neural_training, const char *title_file, const char *object_title)
{
    ...
    if(neural_rendering == 1) {
        ...
        //snprintf(filepath, sizeof(filepath), "db_obj/%s", db_name);
        ...
        /*
        char *model_path = malloc(totalLength);
        if (!model_path) {
        ...
        // Use snprintf to format the string
        bytesWritten = snprintf(model_path, totalLength, "%.*s_%s", dbNameLength - 2, db_name, object_name);
        ...
        */
        ...
        // This is what causes it to not render image with -Z args
        // Need to also pass in the name of the object
        NRTInstance = NeuralRayTracer_Create(model_path_str);

        // printf("instance created\n");
        //printf("MODEL PATH STR%s\n", model_path_str);
