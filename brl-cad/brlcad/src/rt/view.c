...
/* #define DRAW_INDICATOR_LINE 1 */
	...
	if (!ZERO(gamma_corr)) {
        ...
        // a color has already been set here before view pixel
...
{
    //printf("a_color before [0]: %f\n", ap->a_color[0]);
    //printf("a_color before[1]: %f\n", ap->a_color[1]);
    //printf("a_color before[2]: %f\n", ap->a_color[2]);
    struct partition *pp;
    struct hit *hitp;
    struct shadework sw;
    ...
    // Not going in here for normal rt
    ...
    // Not going in this for normal rt
...
vdraw open iray;vdraw params c %2.2x%2.2x%2.2x;vdraw write n 0 %g %g %g;vdraw write n 1 %g %g %g;vdraw send\n",
    ...
    /* sw color is set to 1 before it goes into viewshade. Then that sets the color of the pixel and then 
    that gets stored in a_color. viewshade is defined in shade.c*/



    /* individual shaders must handle reflection & refraction */
    // This is what actually gets called on a normal RT
    (void)viewshade(ap, pp, &sw);

    //printf("in view.c\n");
    
    //printf("%p\n", (void *)pp->pt_regionp->reg_mfuncs->mf_render);
...
out:
    ...
    //printf("a_color after [0]: %f\n", ap->a_color[0]);
    //printf("a_color after[1]: %f\n", ap->a_color[1]);
    //printf("a_color after[2]: %f\n", ap->a_color[2]);
    return 1;
}
