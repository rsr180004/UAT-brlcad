...
viewshade(struct application *ap, const struct partition *pp, struct shadework *swp)
{
    ...
    //printf("mfp render\n");
    //printf(mfp->mf_render);
    /* Invoke the actual shader (may be a tree of them) */
    // This is where the color actually changes for a normal RT (it changes here for swp, which gets stored into a_color)
    if (mfp && mfp->mf_render)
	(void)mfp->mf_render(ap, pp, swp, rp->reg_udata);  

    //printf("Address of mfp->mf_render: %p\n", (void *)mfp->mf_render);
